...
struct dir_Structure* findFiles (unsigned char flag, char *fileName)
{
    ...
    while(1)
    {
                ...
                if((dir->name[0] != DELETED) && (dir->attrib != ATTR_LONG_NAME))
                {
                    ...
                    else  //when flag = GET_LIST
                    {
                        ...
                        if((dir->attrib != 0x10) && (dir->attrib != 0x08))
                        {
                            ...
                            printString ((dir->attrib == 0x10)? "DIR\r\n" : "ROOT\r\n");
                    }
                }
            }
        }
        ...
        if(cluster > 0x0ffffff6)
            return 0;
        if(cluster == 0) 
        {printString("Error in getting cluster");  return 0;}
    }
    return 0;
}
...
unsigned char readFile (unsigned char flag, char *fileName)
{
    ...
    while(1)
    {
        ...
        cluster = getSetNextCluster (cluster, GET, 0);
        if(cluster == 0) {printString("Error in getting cluster"); return 0;}
    }
    return 0;
}
...
{
    unsigned char j, fileCreatedFlag = 0,  sector;
    unsigned int i, firstClusterHigh, firstClusterLow;
    struct dir_Structure *dir;
    unsigned long cluster, prevCluster, firstSector;
    ...
    while(1)
    {
        ...
        if(cluster > 0x0ffffff6)
        {
            ...
            {	
                printString("End of Cluster Chain"); 
                return;
            }
        }
        if(cluster == 0) {printString("Error in getting cluster"); return;}
